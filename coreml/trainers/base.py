...
class Trainer(pl.Trainer):
    ...
    def get_callbacks(self):
        ...
        checkpoint_args = self.config['save']
        checkpoint = pl.callbacks.model_checkpoint.ModelCheckpoint(
            **checkpoint_args)

        return [lr_logger, checkpoint]
    ...
    def fit(self):
        ...
        self.config.module['params'].update(self.config['modes'])
        lightning_module = lm_factory.create(
            self.config.module['name'], **self.config.module['params'])
        super(Trainer, self).fit(lightning_module, dm=data_module)
